      ...
      <section class="right-side">
        <div class="productPage">
          <div class="PageDetails">
            ...
            <h2 v-if="!isDistributor">Products Page</h2>
            <h2 v-else>Our partners</h2>

            <div v-if="!isSeller" class="coffeePage">
              <div class="coffeeBox" v-for="(product, idx) in coffeeList" :key="idx">
                <aside class="productImg">{{ product[1].coffeeName }}</aside>
                <div class="text">
                  <h3 v-if="!isDistributor">Ξ {{ product[1].price }} / <small>100g</small></h3>
                  <button v-if="!isDistributor" type="button" class="btn toModal" @click="showModal(product[1])">Select!</button>
                </div>
              </div>
            </div>

            <div v-else class="coffeePage">
              <div class="coffeeBox" v-for="(product, idx) in filteredCoffeeList" :key="idx">
                <aside class="productImg">{{ product.coffeeName }}</aside>
                <div class="text">
                  <h3 v-if="!isDistributor">Ξ {{ product.price }} / <small>100g</small></h3>
                  <button v-if="!isDistributor" type="button" class="btn toModal" @click="showModal(product)">Select!</button>
                </div>
              </div>
            </div>
...
export default {
  ...
  // 셀러가 관리하는 상품만 필터링
  filteredCoffeeList() {
    return Array.from(this.coffeeList.values()).filter(
      (product) => product.pId === this.pId
    );
  },
},
  methods: {
    ...
    showModal(val) {
      ...
      let coffeeOject = new productClass(val.pId, val.coffeeName, val.price, val.description, '', '', val.origin, '', 0);
